...
import { ExpenseReportDto } from '../../shared/models/water-system.models';
...
export class ExpenseReportComponent implements OnInit {
    breadcrumbItems = [
        ...
        { label: 'Reportes', link: '#' },
        { label: 'Reporte de Egresos', link: '/reports/expenses' }
    ];

    report: ExpenseReportDto | null = null;
    isLoading = false;
    ...
    loadReport(): void {
        ...
        this.isLoading = true;
        this.reportService.getExpenseReport(this.startDate, this.endDate).subscribe({
            next: (data) => {
                this.report = data;
            ...
            error: (error) => {
                console.error('Error al cargar reporte de egresos:', error);
    ...
    exportToExcel(): void {
        ...
        // Add Header Row
        data.push(['Nº', 'FECHA', 'DETALLE', 'TOTAL', 'RESPONSABLE']);
        data.push([]); // Spacer
        ...
        XLSX.writeFile(wb, `Reporte_Egresos_${timestamp}.xlsx`);
